...
class ChatMLDatasetSample:
    ...
    audio_ids_concat: torch.LongTensor  # Shape (num_codebooks, audio_seq_len) or (num_channels, num_codebooks, audio_seq_len)
    ...
    audio_ids_segment_channels: Optional[torch.LongTensor] = None  # Shape (num_audios,)
    # Here `audio_seq_len` is the length of the concatenated audio tokens.`
    ...
    def get_audio_codes(self, idx):
        ...
        if self.audio_ids_concat.dim() == 2:
            return self.audio_ids_concat[:, code_start:code_end]
        elif self.audio_ids_concat.dim() == 3:
            if self.audio_ids_segment_channels is not None:
                channel_idx = int(self.audio_ids_segment_channels[idx].item())
                return self.audio_ids_concat[channel_idx : channel_idx + 1, :, code_start:code_end]
            return self.audio_ids_concat[:, :, code_start:code_end]
        raise ValueError(
            f"Unsupported audio_ids_concat dim {self.audio_ids_concat.dim()}. Expected 2 or 3."
        )
    ...
    def get_audio_codes_labels(self, idx):
        ...
        if self.audio_label_ids_concat.dim() == 2:
            return self.audio_label_ids_concat[:, code_start:code_end]
        elif self.audio_label_ids_concat.dim() == 3:
            if self.audio_ids_segment_channels is not None:
                channel_idx = int(self.audio_ids_segment_channels[idx].item())
                return self.audio_label_ids_concat[channel_idx : channel_idx + 1, :, code_start:code_end]
            return self.audio_label_ids_concat[:, :, code_start:code_end]
        raise ValueError(
            f"Unsupported audio_label_ids_concat dim {self.audio_label_ids_concat.dim()}"
        )
    ...
    def get_audio_segment_channel(self, idx: int) -> Optional[int]:
        if self.audio_ids_segment_channels is None:
            return None
        return int(self.audio_ids_segment_channels[idx].item())
    ...
    def merge(
        ...
        for sample in samples:
            ...
            if sample.audio_ids_segment_channels is not None:
                audio_segment_channels_list.append(sample.audio_ids_segment_channels)
        ...
        try:
            ...
            audio_ids_segment_channels = (
                torch.cat(audio_segment_channels_list, dim=0) if audio_segment_channels_list else None
            )
            ...
            audio_ids_segment_channels = None
