...
class HiggsAudioBatchInput:
    ...
    # Multi-channel position and channel IDs for audio tokens
    audio_out_position_ids: Optional[torch.LongTensor] = None  # shape (audio_out_total_length,)
    audio_out_channel_ids: Optional[torch.LongTensor] = None  # shape (audio_out_total_length,)
    audio_in_position_ids: Optional[torch.LongTensor] = None  # shape (audio_in_total_length,)
    audio_in_channel_ids: Optional[torch.LongTensor] = None  # shape (audio_in_total_length,)
...
class HiggsAudioSampleCollator:
    ...
    Args:
        ...
        interleave_audio_channels (bool): Whether to interleave multi-channel audio tokens when converting to
            a 1D stream. If False, channels are concatenated sequentially.
        audio_token_frame_hz (int): Number of codec frames per second per channel. Used when
            `interleave_audio_channels` is True to determine the interleaving stride (default: 25).
    ...
    def __init__(
        ...
        interleave_audio_channels=False,
        audio_token_frame_hz=25,
    ):
        ...
        self.interleave_audio_channels = interleave_audio_channels
        self.audio_token_frame_hz = audio_token_frame_hz
    ...
    def _generate_audio_position_and_channel_ids(
        self,
        codes: torch.Tensor,
        channel_ids_override: Optional[torch.Tensor] = None,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Generate position IDs and channel IDs for audio codes.

        For interleave mode, tokens from the same time frame across different channels
        will share the same position ID.

        Args:
            codes: Audio code tensor of shape (num_codebooks, seq_len) or
                   (channels, num_codebooks, seq_len)

        Returns:
            position_ids: Shape (seq_len,) - temporal position of each token
            channel_ids: Shape (seq_len,) - channel index of each token
        """
        if channel_ids_override is not None:
            seq_len = codes.shape[-1]
            if isinstance(channel_ids_override, torch.Tensor):
                channel_ids = channel_ids_override.to(dtype=torch.long)
                if channel_ids.dim() == 0:
                    channel_ids = channel_ids.repeat(seq_len)
                elif channel_ids.shape[0] != seq_len:
                    raise ValueError(
                        f"Channel IDs override length {channel_ids.shape[0]} does not match "
                        f"sequence length {seq_len}."
                    )
            else:
                channel_ids = torch.full((seq_len,), int(channel_ids_override), dtype=torch.long)
            position_ids = torch.arange(seq_len, dtype=torch.long)
            return position_ids, channel_ids

        if codes.dim() == 2:
            # Single channel case: (num_codebooks, seq_len)
            seq_len = codes.shape[1]
            position_ids = torch.arange(seq_len, dtype=torch.long)
            channel_ids = torch.zeros(seq_len, dtype=torch.long)
            return position_ids, channel_ids

        if codes.dim() != 3:
            raise ValueError(
                f"Unsupported audio token shape {tuple(codes.shape)}. Expected (codebooks, seq_len) "
                "or (channels, codebooks, seq_len)."
            )

        channels, num_codebooks, seq_len = codes.shape

        if self.interleave_audio_channels:
            frames_per_chunk = max(int(self.audio_token_frame_hz), 1)
            position_ids_list: List[torch.Tensor] = []
            channel_ids_list: List[torch.Tensor] = []

            for start in range(0, seq_len, frames_per_chunk):
                end = min(start + frames_per_chunk, seq_len)
                chunk_len = end - start
                for ch in range(channels):
                    # All tokens in this chunk from this channel share the same position
                    position_ids_list.append(torch.arange(start, end, dtype=torch.long))
                    channel_ids_list.append(torch.full((chunk_len,), ch, dtype=torch.long))

            position_ids = torch.cat(position_ids_list, dim=0)
            channel_ids = torch.cat(channel_ids_list, dim=0)
        else:
            # Concatenate channel streams sequentially
            position_ids_list = []
            channel_ids_list = []
            for ch in range(channels):
                position_ids_list.append(torch.arange(seq_len, dtype=torch.long))
                channel_ids_list.append(torch.full((seq_len,), ch, dtype=torch.long))
    ...
    def _normalize_audio_codes(self, codes: torch.Tensor) -> torch.Tensor:
        """Normalize audio code tensor to 2D (num_codebooks, seq_len).

        Supports incoming tensors of shape:
            - (num_codebooks, seq_len): already normalized.
            - (channels, num_codebooks, seq_len): multi-channel tokens. The channels
              will be interleaved according to `audio_token_frame_hz` when
              `interleave_audio_channels` is enabled, otherwise channels are
        ...
        if self.interleave_audio_channels:
            frames_per_chunk = max(int(self.audio_token_frame_hz), 1)
            # reshape to (num_codebooks, channels, seq_len)
            codes = codes.permute(1, 0, 2)
            interleaved: List[torch.Tensor] = []
            for start in range(0, seq_len, frames_per_chunk):
                end = min(start + frames_per_chunk, seq_len)
                for ch in range(channels):
                    interleaved.append(codes[:, ch, start:end])
            normalized = torch.cat(interleaved, dim=-1)
        else:
            # Concatenate channel streams sequentially
            codes = codes.permute(1, 0, 2)
            normalized = codes.reshape(num_codebooks, channels * seq_len)
    ...
    def __call__(self, batch: List[ChatMLDatasetSample]):
        ...
        # Lists for position and channel IDs
        audio_out_position_ids_l = []
        audio_out_channel_ids_l = []
        audio_in_position_ids_l = []
        audio_in_channel_ids_l = []
        ...
        for i in range(len(processed_batch)):
            ...
            if self.return_audio_in_tokens:
                for idx in audio_in_ids:
                    segment_idx = int(idx.item())
                    codes = processed_batch[i].get_audio_codes(segment_idx)[: self.audio_num_codebooks]
                    channel_override = processed_batch[i].get_audio_segment_channel(segment_idx)
                    # Generate position and channel IDs before normalization
                    pos_ids, ch_ids = self._generate_audio_position_and_channel_ids(
                        codes,
                        channel_override,
                    )
                    audio_in_position_ids_l.append(pos_ids)
                    audio_in_channel_ids_l.append(ch_ids)
                    # Then normalize codes
                    codes = self._normalize_audio_codes(codes)
                    audio_in_ids_l.append(codes)
                if processed_batch[i].audio_label_ids_concat is not None:
                    if audio_in_label_ids_l is None:
                        audio_in_label_ids_l = []
                    for idx in audio_in_ids:
                        segment_idx = int(idx.item())
                        label_codes = processed_batch[i].get_audio_codes_labels(segment_idx)[: self.audio_num_codebooks]
                        label_codes = self._normalize_audio_codes(label_codes)
                        audio_in_label_ids_l.append(label_codes)
            ...
            for idx in audio_out_ids:
                ...
                audio_out_position_ids_l.append(pos_ids)
                audio_out_channel_ids_l.append(ch_ids)
                # Then normalize codes
                codes = self._normalize_audio_codes(codes)
                audio_out_ids_l.append(codes)
            ...
            if processed_batch[i].audio_label_ids_concat is not None:
                ...
                for idx in audio_out_ids:
                    ...
                    label_codes = processed_batch[i].get_audio_codes_labels(segment_idx)[: self.audio_num_codebooks]
                    label_codes = self._normalize_audio_codes(label_codes)
                    audio_out_label_ids_l.append(label_codes)
        ...
        if len(audio_in_ids_l) > 0:
            ...
            # Concatenate position and channel IDs
            audio_in_position_ids = torch.cat(audio_in_position_ids_l, dim=0).long() if audio_in_position_ids_l else None
            audio_in_channel_ids = torch.cat(audio_in_channel_ids_l, dim=0).long() if audio_in_channel_ids_l else None
        else:
            audio_in_ids = torch.zeros((0, 0), dtype=torch.long)
            audio_in_ids_start = torch.zeros(0, dtype=torch.long)
            audio_in_position_ids = None
            audio_in_channel_ids = None
        ...
        if len(audio_out_ids_l) > 0:
            ...
            # Concatenate position and channel IDs
            audio_out_position_ids = torch.cat(audio_out_position_ids_l, dim=0).long() if audio_out_position_ids_l else None
            audio_out_channel_ids = torch.cat(audio_out_channel_ids_l, dim=0).long() if audio_out_channel_ids_l else None
        else:
            audio_out_ids = torch.zeros((0, 0), dtype=torch.long)
            audio_out_ids_start = torch.zeros(0, dtype=torch.long)
            audio_out_position_ids = None
            audio_out_channel_ids = None
            if return_labels:
        ...
        return HiggsAudioBatchInput(
            ...
            audio_out_position_ids=audio_out_position_ids,
            audio_out_channel_ids=audio_out_channel_ids,
            audio_in_position_ids=audio_in_position_ids,
            audio_in_channel_ids=audio_in_channel_ids,
            reward=reward,
        )
