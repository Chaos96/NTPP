...
class HiggsAudioModel(HiggsAudioPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: HiggsAudioConfig):
        ...
        # Channel embedding for multi-channel audio
        if config.use_audio_channel_embedding:
            self.audio_channel_embeddings = nn.Embedding(
                config.num_audio_channels, config.text_config.hidden_size
            )
        else:
            self.audio_channel_embeddings = None
    ...
    def _embed_audio_ids(self, audio_ids, channel_ids=None):
        """Embed the audio ids

        Args:
            audio_ids: torch.LongTensor of shape (num_codebooks, audio_in_total_length)
            channel_ids: Optional[torch.LongTensor] of shape (audio_in_total_length,)
                        Channel index for each audio token. If provided and
                        use_audio_channel_embedding is enabled, channel embeddings
                        will be added to audio embeddings.
        ...
        # Add channel embeddings if enabled and channel_ids provided
        if self.audio_channel_embeddings is not None and channel_ids is not None:
            # Pad channel_ids if length doesn't match (e.g., BOS/EOS tokens added)
            if channel_ids.shape[0] != audio_embed.shape[0]:
                # Pad with channel 0 for BOS/EOS tokens
                pad_length = audio_embed.shape[0] - channel_ids.shape[0]
                channel_ids = torch.cat([
                    torch.zeros(pad_length // 2, dtype=channel_ids.dtype, device=channel_ids.device),
                    channel_ids,
                    torch.zeros(pad_length - pad_length // 2, dtype=channel_ids.dtype, device=channel_ids.device)
                ])
            channel_embed = self.audio_channel_embeddings(channel_ids)
    ...
    def forward(
        ...
        audio_out_position_ids: Optional[torch.LongTensor] = None,
        audio_out_channel_ids: Optional[torch.LongTensor] = None,
        audio_in_position_ids: Optional[torch.LongTensor] = None,
        audio_in_channel_ids: Optional[torch.LongTensor] = None,
    ):
        ...
        if self.config.encode_audio_in_tokens:
            ...
            # Transfer channel_ids to device if provided
            if audio_in_channel_ids is not None:
                audio_in_channel_ids = audio_in_channel_ids.to(target_device)
            audio_in_embed = self._embed_audio_ids(audio_in_ids, channel_ids=audio_in_channel_ids)
        else:
            audio_in_embed = None
        ...
        # Transfer channel_ids to device if provided
        if audio_out_channel_ids is not None:
            audio_out_channel_ids = audio_out_channel_ids.to(target_device)
        audio_out_embed = self._embed_audio_ids(audio_out_ids, channel_ids=audio_out_channel_ids)
